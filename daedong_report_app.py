...
st.sidebar.subheader(f"⚙️ {current_month}월 차트 음영 기준치 (수정 가능)")
thresholds = {}
for metric in selected_metrics:
    st.sidebar.markdown(f"**{metric}**")
    col1, col2 = st.sidebar.columns(2)
    if "온도" in metric:
        min_v = col1.number_input("Min", value=round(month_guide['avg_t'] - 2.0, 1), step=0.5, key=f"min_{metric}")
        max_v = col2.number_input("Max", value=round(month_guide['avg_t'] + 2.0, 1), step=0.5, key=f"max_{metric}")
    elif "습도" in metric:
        min_v = col1.number_input("Min", value=round(max(0, month_guide['hum'] - 10.0), 1), step=1.0, key=f"min_{metric}")
        max_v = col2.number_input("Max", value=round(min(100, month_guide['hum'] + 10.0), 1), step=1.0, key=f"max_{metric}")
    else:
        min_v = col1.number_input("Min", value=round(max(0, month_guide['co2'] - 50.0), 1), step=10.0, key=f"min_{metric}")
        max_v = col2.number_input("Max", value=round(month_guide['co2'] + 50.0, 1), step=10.0, key=f"max_{metric}")
    thresholds[metric] = (min_v, max_v)
...
col_logo, col_empty, col_info = st.columns([1.5, 0.5, 2.5])
with col_logo:
    if os.path.exists("image_8a62a0.png"):
...
def render_summary_card(title, act_val, guide_val, unit):
    ...
    <div style="background-color: {DAEDONG['dark_gray']}; padding: 18px; border-radius: 8px;
        ...
        <div style="font-size: 14px; color: {DAEDONG['medium_gray']}; margin-bottom: 10px;">가이드: {guide_val}{unit}</div>
